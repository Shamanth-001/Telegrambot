# Complete Auto-Run System for Einthusan Only
# Checks ByeDPI, fixes issues, and auto-tests Ulidavaru Kandanthe until successful

Write-Host "EINTHUSAN ONLY AUTO-RUN SYSTEM STARTING..." -ForegroundColor Green
...
function Test-EinthusanAccess {
    ...
    try {
        $response = Invoke-WebRequest -Uri "https://einthusan.tv" -TimeoutSec 15 -UseBasicParsing
        if ($response.StatusCode -eq 200) {
            Write-Host "Einthusan.tv is accessible (Status: $($response.StatusCode), Size: $($response.Content.Length) chars)" -ForegroundColor Green
            return $true
        } else {
...
function Test-EinthusanSearch {
    Write-Host "Testing Einthusan search for Ulidavaru Kandanthe..." -ForegroundColor Yellow
    
    try {
        $searchUrl = "https://einthusan.tv/movie/results/?lang=kannada&query=Ulidavaru%20Kandanthe"
        $response = Invoke-WebRequest -Uri $searchUrl -TimeoutSec 15 -UseBasicParsing
        if ($response.StatusCode -eq 200) {
            Write-Host "Einthusan search accessible (Status: $($response.StatusCode), Size: $($response.Content.Length) chars)" -ForegroundColor Green
            return $true
        } else {
            Write-Host "Einthusan search returned status: $($response.StatusCode)" -ForegroundColor Yellow
            return $false
        }
    } catch {
        Write-Host "Einthusan search failed: $($_.Exception.Message)" -ForegroundColor Red
        return $false
    }
}
...
function Start-BotAndTest {
    ...
    Write-Host "EINTHUSAN ONLY MODE - All other sources disabled" -ForegroundColor Magenta
...
function Main {
    Write-Host "EINTHUSAN ONLY AUTO-RUN SYSTEM" -ForegroundColor Green
    Write-Host "===============================" -ForegroundColor Green
    ...
    do {
        ...
        Write-Host "Testing Einthusan.tv accessibility..." -ForegroundColor Yellow
        $einthusanAccess = Test-EinthusanAccess
        
        if ($einthusanAccess) {
            Write-Host "Testing Einthusan search functionality..." -ForegroundColor Yellow
            $searchAccess = Test-EinthusanSearch
            
            if ($searchAccess) {
                Write-Host "Einthusan is fully accessible!" -ForegroundColor Green
                
                # Step 3: Start bot and test
                Write-Host "Starting bot test..." -ForegroundColor Yellow
                Start-BotAndTest
                
                Write-Host "Bot test completed - check Telegram for results" -ForegroundColor Green
                return
            } else {
                Write-Host "Einthusan search not working - retrying..." -ForegroundColor Yellow
            }
        } else {
            Write-Host "Einthusan.tv not accessible - retrying..." -ForegroundColor Yellow
        }
    ...
    Write-Host "All attempts failed - Einthusan.tv is blocked" -ForegroundColor Red
    Write-Host "Try mobile deployment or alternative VPN" -ForegroundColor Yellow
}
