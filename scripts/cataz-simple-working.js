...
// Simple Working Cataz Downloader
class CatazSimpleWorking {
  constructor() {
    this.browser = null;
    this.page = null;
  ...
  async downloadMovie(movieUrl, movieTitle) {
    try {
      this.logger.info(`🎬 Starting Cataz simple download for: ${movieTitle}`);
      ...
      for (let i = 0; i < this.capturedStreams.length; i++) {
        ...
        const outputPath = `downloads/${movieTitle.replace(/[^a-zA-Z0-9]/g, '_')}-simple-${i + 1}.mp4`;
        const result = await this.downloadWithYtdlp(stream.url, outputPath);
        
        if (result.success) {
          this.logger.success(`🎉 Cataz simple download completed!`);
      ...
      throw new Error('All Cataz simple download attempts failed');

    } catch (error) {
      this.logger.error(`❌ Cataz simple download failed: ${error.message}`);
      throw error;
    } finally {
      if (this.browser) {
...
// Test the Cataz simple downloader
async function downloadFromCatazSimple() {
  console.log('🎬 Cataz Simple Working Downloader');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('✅ VPN Status: Active (optimized)');
  console.log('🔧 Configuration: VPN-friendly with simple stream capture');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

  const downloader = new CatazSimpleWorking();

  try {
    const result = await downloader.downloadMovie(
      'https://cataz.to/movie/watch-avatar-2009-19690',
      'Avatar_2009_Cataz_Simple'
    );
    
    console.log('🎉 Cataz simple download completed successfully!');
    console.log(`📁 File: ${result.filePath}`);
    console.log(`📊 Size: ${(result.fileSize / 1024 / 1024).toFixed(2)} MB`);
    console.log('🛡️ Your VPN is working perfectly with Cataz simple streams!');
    
  } catch (error) {
    console.error('❌ Cataz simple download failed:', error.message);
    console.log('💡 Tips:');
...
// Run the download
downloadFromCatazSimple().catch(console.error);
