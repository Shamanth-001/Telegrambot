...
// Direct Cataz Downloader
class CatazDirectDownloader {
  constructor() {
    this.browser = null;
    this.page = null;
  ...
  async downloadMovie(movieUrl, movieTitle) {
    try {
      this.logger.info(`🎬 Starting Cataz direct download for: ${movieTitle}`);
      ...
      for (let i = 0; i < this.capturedStreams.length; i++) {
        ...
        const outputPath = `downloads/${movieTitle.replace(/[^a-zA-Z0-9]/g, '_')}-direct-${i + 1}.mp4`;
        const result = await this.downloadWithVPN(stream.url, outputPath);
        
        if (result.success) {
          this.logger.success(`🎉 Cataz direct download completed!`);
      ...
      throw new Error('All Cataz direct download attempts failed');

    } catch (error) {
      this.logger.error(`❌ Cataz direct download failed: ${error.message}`);
      throw error;
    } finally {
      if (this.browser) {
...
// Test the Cataz direct downloader
async function downloadFromCatazDirect() {
  console.log('🎬 Cataz Direct Stream Downloader');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('✅ VPN Status: Active (optimized)');
  console.log('🔧 Configuration: VPN-friendly with direct stream capture');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

  const downloader = new CatazDirectDownloader();

  try {
    const result = await downloader.downloadMovie(
      'https://cataz.to/movie/watch-avatar-2009-19690',
      'Avatar_2009_Cataz_Direct'
    );
    
    console.log('🎉 Cataz direct download completed successfully!');
    console.log(`📁 File: ${result.filePath}`);
    console.log(`📊 Size: ${(result.fileSize / 1024 / 1024).toFixed(2)} MB`);
    console.log('🛡️ Your VPN is working perfectly with Cataz direct streams!');
    
  } catch (error) {
    console.error('❌ Cataz direct download failed:', error.message);
    console.log('💡 Tips:');
...
// Run the download
downloadFromCatazDirect().catch(console.error);
