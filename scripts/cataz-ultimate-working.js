...
// Ultimate Working Cataz Downloader
class CatazUltimateWorking {
  constructor() {
    this.browser = null;
    this.page = null;
  ...
  async processIframeStream(iframeUrl) {
    this.logger.info(`🔍 Processing iframe stream: ${iframeUrl}`);
    
    try {
      // Navigate to the iframe URL
      await this.page.goto(iframeUrl, { waitUntil: 'networkidle2' });
      this.logger.success('✅ Iframe page loaded');
      
      // Wait for content to load
      await new Promise(resolve => setTimeout(resolve, 10000));
      
      // Try to find video elements in the iframe
      try {
        ...
        if (videoElements.length > 0) {
          for (const video of videoElements) {
            ...
            if (src && src !== 'blob:') {
              ...
              this.logger.info(`🎬 Iframe video stream: ${src}`);
            }
          }
        }
      } catch (error) {
            ...
            if (urlMatches) {
              for (const url of urlMatches) {
                this.capturedStreams.push({
                  url: url,
                  timestamp: new Date().toISOString(),
                  source: 'iframe-script'
                });
                this.logger.info(`🎬 Iframe script stream: ${url}`);
              }
            }
          }
        }
      } catch (error) {
        this.logger.info('No script streams found in iframe');
      }

      return this.capturedStreams;
      
    } catch (error) {
      this.logger.error(`Failed to process iframe: ${error.message}`);
      return this.capturedStreams;
    }
  }
  ...
  async downloadMovie(movieUrl, movieTitle) {
    try {
      this.logger.info(`🎬 Starting Cataz ultimate download for: ${movieTitle}`);
      ...
      if (iframes.length > 0) {
        for (const iframe of iframes) {
          ...
          if (src && (src.includes('player') || src.includes('embed') || src.includes('videostr'))) {
            ...
            // Process the iframe stream
            await this.processIframeStream(src);
          }
        }
      }
      
      if (this.capturedStreams.length === 0) {
        throw new Error('No stream URLs captured from iframe');
      }

      this.logger.success(`🎬 Captured ${this.capturedStreams.length} stream URLs from iframe`);
      ...
      for (let i = 0; i < this.capturedStreams.length; i++) {
        ...
        const outputPath = `downloads/${movieTitle.replace(/[^a-zA-Z0-9]/g, '_')}-ultimate-${i + 1}.mp4`;
        const result = await this.downloadWithYtdlp(stream.url, outputPath);
        
        if (result.success) {
          this.logger.success(`🎉 Cataz ultimate download completed!`);
      ...
      throw new Error('All Cataz ultimate download attempts failed');

    } catch (error) {
      this.logger.error(`❌ Cataz ultimate download failed: ${error.message}`);
      throw error;
    } finally {
      if (this.browser) {
...
// Test the Cataz ultimate downloader
async function downloadFromCatazUltimate() {
  console.log('🎬 Cataz Ultimate Working Downloader');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('✅ VPN Status: Active (optimized)');
  console.log('🔧 Configuration: VPN-friendly with ultimate iframe processing');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

  const downloader = new CatazUltimateWorking();

  try {
    const result = await downloader.downloadMovie(
      'https://cataz.to/movie/watch-avatar-2009-19690',
      'Avatar_2009_Cataz_Ultimate'
    );
    
    console.log('🎉 Cataz ultimate download completed successfully!');
    console.log(`📁 File: ${result.filePath}`);
    console.log(`📊 Size: ${(result.fileSize / 1024 / 1024).toFixed(2)} MB`);
    console.log('🛡️ Your VPN is working perfectly with Cataz ultimate streams!');
    
  } catch (error) {
    console.error('❌ Cataz ultimate download failed:', error.message);
    console.log('💡 Tips:');
...
// Run the download
downloadFromCatazUltimate().catch(console.error);
